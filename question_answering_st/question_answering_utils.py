...
def squad_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length, padding_strategy, is_training):
    ...
    for span in spans:
        ...
        N=40
        kg_enc = parse_triplet(example.kg_input,N)
        # kg_enc = example.kg_input[:N]
        # add_special_tokens=False, truncation=True, max_length=max_query_length)
        truncated_kg_input = tokenizer.encode(kg_enc, max_length=512, padding='max_length', add_special_tokens=True, truncation=True)
        # print("\n",truncated_kg_input)
        # print("\n",kg_enc)
        # print("\n",example.qas_id)
