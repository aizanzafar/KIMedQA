...
class roberta_model(RobertaForQuestionAnswering):
    def __init__(self, RobertaConfig):
        ...
        print("no of labels: ",RobertaConfig.num_labels)
        ...
        self.linear_layer = nn.Linear(RobertaConfig.hidden_size*3,RobertaConfig.hidden_size)
    ...
    def forward(
        ...
        kg_outputs = self.roberta(kg_input_ids)
        kg_sequence_output = kg_outputs[0]

        # print("triple_emb: ", kg_sequence_output.size()) #triple_emb:  torch.Size([32, 100, 768]) bs,seq_len, 768

        ### Only for ablation without attention mechanism
        mlp= torch.cat([sequence_output,kg_sequence_output],dim=1)
        # print(mlp.shape)
        mlp_1= torch.permute(mlp,(0,2,1))
        # print(mlp_1.shape)
        mlp_out = self.linear_concat(mlp_1)
        # print(mlp_out.shape)
        join_output= torch.permute(mlp_out,(0,2,1))
        # print(join_output.shape)
